...
def load_best_fitness(json_path):
    ...
    Parse the 'test_fitness' field for the median, track the maximum across all valid generations.
    ...
    for entry in data:
        ...
        if isinstance(gen_val, str) and gen_val.lower() == "initial":
            continue
        # Use the 'test_fitness' field now instead of 'fitness'
        test_fit_str = entry.get("test_fitness", "")
        fit_val = parse_median_fitness(test_fit_str)
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute average highest test fitness and std for each (benchmark, algorithm) from subfolders."
    )
